...
module.exports.validateOptions = (options, type) => {
  if (!options) throw "Missing arguments: options is undefined.";
  ...
  else options.user = client.resolver.resolveUser(options.user);
  if (!options.user) throw "Invalid input: user is undefined or invalid.";
  ...
  if (options.botMessage.channel.type == "dm") options.deleteReaction = false;
  else if (type === "reactMenu") options.deleteReaction = true;
  else if (options.deleteReaction === undefined) options.deleteReaction = true;
  else if (!isBoolean(options.deleteReaction))
    options.deleteReaction = Boolean(options.deleteReaction);

  if (options.botMessage.channel.type == "dm") options.deleteAllOnEnd = false;
  else if (type === "reactMenu" || type === "reactPaginator")
    options.deleteAllOnEnd = true;
  else if (options.deleteAllOnEnd === undefined) options.deleteAllOnEnd = true;
  ...
  switch (type) {
    ...
    case "messageAsyncQuestion":
      ...
      if (
        options.botMessage.channel.type == "text" &&
