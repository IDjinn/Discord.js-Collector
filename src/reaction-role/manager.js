...
class ReactionRoleManager extends EventEmitter {
  ...
  constructor(
    ...
    this.client.on("roleDelete", async (role) => {
      const reactionRole = this.reactionRoles.find(
        (reactionRole) => reactionRole.role == role.id
      );
      if (reactionRole) return await this.__handleDeleted(reactionRole, role);
    });

    this.client.on("emojiDelete", async (emoji) => {
      const emojiIdentifier = this.__resolveReactionEmoji(emoji);
      const reactionRole = this.reactionRoles.find(
        (reactionRole) => reactionRole.emoji == emojiIdentifier
      );
      if (reactionRole) return await this.__handleDeleted(reactionRole, emoji);
    });

    this.client.on("guildDelete", async (guild) => {
      const reactionRole = this.reactionRoles.find(
        (reactionRole) => reactionRole.guild == guild.id
      );
      if (reactionRole) return await this.__handleDeleted(reactionRole, guild);
    });

    this.client.on("channelDelete", async (channel) => {
      const reactionRole = this.reactionRoles.find(
        (reactionRole) => reactionRole.channel == channel.id
      );
      if (reactionRole)
    ...
    const messageDeleteHandler = async (message) => {
      const reactionRole = this.reactionRoles.find(
        (reactionRole) => reactionRole.message == message.id
      );
      if (reactionRole)
  ...
  async __handleDeleted(reactionRole, guildResolvable) {
    ...
    const reaction = message.reactions.cache.find(
      (x) =>
        reactionRole.id == `${message.id}-${this.__resolveReactionEmoji(x)}`
    );
  ...
  async __resfreshOnBoot() {
    ...
    for (const reactionRole of this.reactionRoles.values()) {
      ...
      if (!message.reactions.cache.has(reactionRole.emoji))
        // Bot reaction is deleted, and not have any reaction with this reaction role emoji.
  ...
  async __checkRequirements(reactionRole, reaction, member) {
    return new Promise(async (resolve) => {
      if (!reactionRole.checkBoostRequirement(member)) {
        ...
        this.__debug(
          "BOOT",
          `Member '${user.id}' not have boost requirement, will not win this role.`
        );
  ...
  createReactionRole(
    ...
    return new Promise(async (resolve, reject) => {
      if (message instanceof Message) {
        ...
        if (!role) return reject("Bad input: I canno't resolve role " + role);
      ...
      return reject(
        "Bad input: addRole({...}) message must be a Message Object."
      );
    });
  }
  ...
  async deleteReactionRole(role, deleted = false) {
    return new Promise(async (resolve, reject) => {
      ...
      return reject(
        "Bad input: removeRole(role) must be a ReactionRole Object."
      );
    });
  }
  ...
  async __parseStorage() {
    return new Promise(async (resolve) => {
      if (this.storage) {
        ...
        for (const role of roles) {
          if (!role || !role.message || role.disabled)
            // TODO: Temporary, need find where have update/insert mongoose error.
            continue;
    ...
    this.timeouts.set(
      ...
      setTimeout(async () => {
        ...
        const toggledRoles = this.reactionRoles.filter(
          (rr) => rr.message == message.id && rr.toggle
        );
        for (const toggledRole of toggledRoles.values()) {
          ...
          const reaction = message.reactions.cache.find(
            (reaction) =>
              this.__resolveReactionEmoji(reaction.emoji) == toggledRole.emoji
          );
          await reaction.users.remove(member.user);
          this.__debug(
            "REACTION",
        ...
        if (skippedRole instanceof ReactionRole) {
          const reaction = message.reactions.cache.find(
            (reaction) =>
              this.__resolveReactionEmoji(reaction.emoji) == skippedRole.emoji
          );
  ...
  async __onRemoveAllReaction(message) {
    const messageReactionsRoles = this.reactionRoles.filter(
      (r) => r.message == message.id
    );
    ...
    const rolesAffected = messageReactionsRoles.map((rr) =>
      message.guild.roles.cache.get(rr.role)
    );
    this.emit(
